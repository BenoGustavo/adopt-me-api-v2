...
describe("OngService", () => {
	...
	describe("create", () => {
		it("should throw UserAlreadyExistsError if user already exists", async () => {
			ongRepository.findByEmail.mockResolvedValue({
				id: "1",
				name: "Test User",
			...
			await expect(ongService.create(data)).rejects.toThrow(
				UserAlreadyExistsError,
			);
		});
		...
		it("should create a new user with hashed password", async () => {
			...
			ongRepository.create.mockResolvedValue({
				id: "1",
				name: "Test User",
				email: "test@example.com",
				whatsapp: "123456789",
			...
			expect(ongRepository.create).toHaveBeenCalledWith({
				...data,
				password: "hashedPassword",
			});
	...
	describe("authenticate", () => {
		it("should throw UserInvalidCredentialsError if user does not exist", async () => {
			...
			await expect(
				ongService.authenticate("test@example.com", "password"),
		...
		it("should return user with token if credentials are correct", async () => {
			...
			const result = await ongService.authenticate(
				"test@example.com",
				"password",
			);
