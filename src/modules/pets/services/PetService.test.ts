...
describe('PetService', () => {
    ...
    beforeEach(() => {
        ...
        petRepository = new PetRepository(prismaClient) as jest.Mocked<PetRepository>;
        ongRepository = new OngRepository(prismaClient) as jest.Mocked<OngRepository>;
        userRepository = new UserRepository(prismaClient) as jest.Mocked<UserRepository>;
        petService = new PetService(petRepository, ongRepository, userRepository);
    });
    ...
    describe('create', () => {
        it('should create a pet if ong exists', async () => {
            ...
            ongRepository.findById.mockResolvedValue({ id: ongId, name: 'Ong', email: 'asdasda@gmail.com', addressId: 'address-id', created_at: new Date(), updated_at: new Date(), whatsapp: '123456789',password: "2342424234" });
        ...
        it('should throw UnauthorizedError if ong does not exist', async () => {
            ...
            await expect(petService.create(ongId, data)).rejects.toThrow(UnautorizedError);
        });
    });
    ...
    describe('findPets', () => {
        it('should find pets with filters', async () => {
            ...
            const result = await petService.findPets(city, size, age, breed, type);

            expect(result).toEqual(pets);
            expect(petRepository.findWithFilters).toHaveBeenCalledWith(city, size, age, breed, type);
        });
    });
    ...
    describe('adoptPet', () => {
        it('should adopt a pet if user and ong exist', async () => {
            ...
            expect(ongRepository.findById).toHaveBeenCalledWith(ongId);
            expect(petRepository.toggleAdoptionStatus).toHaveBeenCalledWith(petId, userId);
        });
        ...
        it('should throw UnauthorizedError if user does not exist', async () => {
            ...
            await expect(petService.adoptPet(petId, userId, ongId)).rejects.toThrow(UnautorizedError);
        });
    ...
    describe('getAdoptedPets', () => {
        ...
        it('should throw UnautorizedError if ong does not exist', async () => {
            ...
            await expect(petService.getAdoptedPets(ongId)).rejects.toThrow(UnautorizedError);
        });
    });
    ...
    describe('deletePet', () => {
        ...
        it('should throw UnautorizedError if ong does not exist', async () => {
            ...
            await expect(petService.deletePet(petId, ongId)).rejects.toThrow(UnautorizedError);
        });
    });
});